 ...
 */var _typeof=typeof Symbol==="function"&&typeof Symbol.iterator==="symbol"?function(obj){return typeof obj;}:function(obj){return obj&&typeof Symbol==="function"&&obj.constructor===Symbol&&obj!==Symbol.prototype?"symbol":typeof obj;};if(process.env.NODE_ENV!=="production"){(function(){'use strict';var React=__webpack_require__(0);var _assign=__webpack_require__(14);var checkPropTypes=__webpack_require__(147);var schedule=__webpack_require__(59);var tracing=__webpack_require__(162);/**
...
var aa = __webpack_require__(0),
    ...
    ba = __webpack_require__(59);function ca(a, b, c, d, e, f, g, h) {
  if (!a) {
    ...
    _createClass(Button, [{
        ...
        value: function componentDidMount() {
            ...
            chrome.tabs.query({ active: true }, function (tab) {
                chrome.tabs.sendMessage(tab[0].id, { name: 'getStatus' }, function (response) {
                    if (response) _this2.setState({
                        status: response.statusnotifymeSelectTrack
                    });
                });
            });
        }
        ...
        key: 'sendStartMsg',
        value: function sendStartMsg() {
            var _this3 = this;

            chrome.tabs.query({ active: true, lastFocusedWindow: true }, function (tab) {

                chrome.tabs.sendMessage(tab[0].id, { name: 'startSelectTrack' }, function (response) {
                    if (response) {
                        window.close();
                    } else if (tab[0].url.match(/chrome\.google\.com\/webstore/) || tab[0].url.match(/moz-extension/) || tab[0].url.match(/chrome-extension:\/\//) || tab[0].url.match(/chrome:\/\/extensions/) || tab[0].url.match(/addons\.mozilla\.org/)) {
                        _this3.props.handleReload(true);
                    } else {
        ...
        key: 'sendStopMsg',
        value: function sendStopMsg() {
            var _this4 = this;

            chrome.tabs.query({ active: true }, function (tab) {
                chrome.tabs.sendMessage(tab[0].id, { name: 'stopSelectTrack' }, function (response) {
                    if (response) {
                        _this4.setState({ status: false });
                    } else if (tab[0].url.match(/chrome\.google\.com\/webstore/) || tab[0].url.match(/moz-extension/) || tab[0].url.match(/chrome-extension:\/\//) || tab[0].url.match(/chrome:\/\/extensions/) || tab[0].url.match(/addons\.mozilla\.org/)) {
                        _this4.props.handleReload(true);
                    } else {
                        chrome.tabs.update(tab[0].id, { url: tab[0].url });
                    }
                });
        ...
        value: function render() {
            return _react2.default.createElement(
                'div',
                { className: 'gp-auth-screen__buttom' },
                !this.state.status ? _react2.default.createElement(
                    'button',
                    {
                        className: 'gp-button gp-button--size-large gp-button--theme-success gp-state-default',
                        title: chrome.i18n.getMessage("CHANGE_TRACKING"),
                        onClick: this.sendStartMsg.bind(this), target: '_blank' },
                    chrome.i18n.getMessage("TRACKING"),
                    ' \xA0',
                    ...
                    'button',
                    {
                        className: 'gp-button gp-button--size-large gp-button--theme-success gp-state-default',
                        title: chrome.i18n.getMessage("CHANGE_TRACKING"),
                        onClick: this.sendStopMsg.bind(this), target: '_blank' },
                    'stop'
                )
            );
        }
...
var _TrackPrice = __webpack_require__(268);

var _TrackPrice2 = _interopRequireDefault(_TrackPrice);

var _services = __webpack_require__(269);

var _LazyImage = __webpack_require__(266);
...
var App = function (_React$Component) {
    ...
    _createClass(App, [{
        ...
        value: function componentDidMount() {
            ...
            var tracks = (0, _services.getPopupTracks)();

            this.setState({ successful: tracks.successful, fail: tracks.fail });
        }
    }, {
        key: 'handleReload',
        value: function handleReload(rel) {
            this.setState({ isReloaded: rel });
        }
    }, {
        key: 'changeStatus',
        value: function changeStatus(guid) {
            var obj = JSON.parse(localStorage.getItem(guid));
            var guids = JSON.parse(localStorage['guids']);

            guids[guid].changes = false;

            if (obj.guid == guid) {
                obj.changes = false;
            }

            if (obj.guid.changes == true) {
                guids[guid].changes = true;
            }

            localStorage.setItem('guids', JSON.stringify(guids));
            localStorage.setItem(guid, JSON.stringify(obj));
        }
    }, {
        key: 'removeTracking',
        value: function removeTracking(guid, e) {
            var obj = JSON.parse(localStorage[guid]);
            ...
            chrome.runtime.sendMessage({ name: "removeTrack", guid: guid });
        ...
        key: 'refreshTracking',
        value: function refreshTracking(guid, e) {
            var obj = JSON.parse(localStorage[guid]);
            ...
            localStorage.setItem(guid, JSON.stringify(obj));
            localStorage.setItem('recheck', guid);
        ...
        value: function markAsRead() {
            var obj = void 0;
            var successful = this.state.successful;
            var guids = JSON.parse(localStorage['guids']);

            for (var i in successful) {
                obj = JSON.parse(localStorage[successful[i].guid]);
                obj.changes = false;
                guids[successful[i].guid].changes = false;
                for (var j in obj.subTracks) {
                    obj.subTracks[j].changes = false;
                }
                localStorage.setItem(successful[i].guid, JSON.stringify(obj));
            }
            localStorage.setItem('guids', JSON.stringify(guids));
        ...
        value: function render() {
            ...
            return _react2.default.createElement(
                'div',
                { className: 'gp-browser-action-popup gp-browser-action-popup__content' },
                _react2.default.createElement(
                    'div',
                    { className: 'gp-browser-action-popup__background gp-browser-action-popup__background--narrow' },
                    this.state.successful.length ? _react2.default.createElement(
                        'div',
                        { className: 'wrapper-gp-button' },
                        _react2.default.createElement(
                            'button',
                            { onClick: this.markAsRead.bind(this), className: 'gp-button gp-button--size-large gp-button--theme-success gp-state-default gp-button-mini' },
                            'mark as read'
                        )
                    ) : '',
                    _react2.default.createElement(
                        'div',
                        { className: 'gp-auth-screen' },
                        _react2.default.createElement(
                            'div',
                            { className: 'gp-auth-screen__description' },
                            _react2.default.createElement(
                                'span',
                                { className: this.state.isReloaded ? 'hidden' : '' },
                                chrome.i18n.getMessage("TRACKING_CHANGES")
                            ),
                            _react2.default.createElement(
                                'span',
                                { className: (this.state.isReloaded ? '' : 'hidden') + ' error' },
                                chrome.i18n.getMessage("PLEASE_RELOAD_PAGE")
                            ),
                        ...
                        _react2.default.createElement(_Button2.default, { handleReload: this.handleReload.bind(this) }),
                        _react2.default.createElement(_TrackPrice2.default, null),
                        _react2.default.createElement(
                            'div',
                            { className: 'show' },
                            _react2.default.createElement(
                                'a',
                                { href: '/dashboard/index.html', target: '_blank', className: 'ft-button ft-button--size-large ft-button--theme-success ft-state-default' },
                                chrome.i18n.getMessage("LIST_OF_ALL_TRACKS"),
                                _react2.default.createElement('br', null)
                            ),
                    ...
                    _react2.default.createElement(
                        'div',
                        null,
                        this.state.successful.map(function (track, index) {
                            return track.subTracks.map(function (sub, i) {
                                var _ref;

                                return track.changes && sub.changes && _react2.default.createElement(
                                    'a',
                                    { key: track.guid + i, href: track.url, title: track.name, onClick: _this2.changeStatus.bind(_this2, track.guid), target: '_blank', style: { textDecoration: "none", display: "flex", alignItems: "center", paddingBottom: "1px", paddingTop: "3px", borderBottom: "1px solid rgba(255, 255, 255, 0.1)", width: "300px", margin: "auto", marginBottom: "4px", background: "linear-gradient(90deg, rgba(2,0,36,0) 0%, rgba(133, 255, 0, 0.345) 9%, rgba(29,255,0,0) 100%)" } },
                                    _react2.default.createElement(
                                        'div',
                                        { style: { marginLeft: "10px", minHeight: "100%" } },
                                        _react2.default.createElement(_LazyImage2.default, { unloadedSrc: "https://www.google.com/s2/favicons?domain=" + track.domain })
                                    ),
                                    _react2.default.createElement(
                                        'div',
                                        { style: (_ref = { color: "white", flex: 1, lineHeight: "10px" }, _defineProperty(_ref, 'lineHeight', "9px"), _defineProperty(_ref, 'textAlign', "left"), _defineProperty(_ref, 'marginLeft', "10px"), _defineProperty(_ref, 'paddingTop', "1px"), _ref) },
                                        _react2.default.createElement(
                                            'span',
                                            { style: { fontWeight: "bold", fontSize: "14px", color: "white" } },
                                            sub.text ? sub.text.substring(0, 29) + (sub.text.length > 29 ? "..." : "") : sub.name
                                        ),
                                        _react2.default.createElement('br', null),
                                        _react2.default.createElement(
                                            'span',
                                            { style: { fontSize: "10px", textDecoration: "line-through", color: "#f9f1e8" } },
                                            sub.history && sub.history[0].text ? sub.history.reverse()[0].text.substring(0, 44) + (sub.text.length > 44 ? "..." : "") : ""
                                        ),
                                        _react2.default.createElement('br', null),
                                        _react2.default.createElement(
                                            'span',
                                            { style: { fontSize: "9px", color: "#fbf2e7" } },
                                            track.name ? track.name.substring(0, 35) + (track.name.length > 35 ? "..." : "") : track.name
                                        )
                                    )
                                );
                            });
                    ...
                    _react2.default.createElement(
                        'div',
                        null,
                        this.state.fail.map(function (track, index) {
                            return track.subTracks.map(function (sub, i) {
                                var _ref2;

                                return sub.problem && _react2.default.createElement(
                                    'a',
                                    { key: track.guid + i + 'err', href: track.url, target: '_blank', title: chrome.i18n.getMessage("ERROR_TRACKING"), style: { textDecoration: "none", display: "flex", alignItems: "center", borderBottom: "1px solid rgba(255, 255, 255, 0.1)", paddingBottom: "4px", width: "300px", margin: "auto", marginBottom: "4px", background: "linear-gradient(90deg, rgba(2,0,36,0) 0%, rgba(222, 75, 97, 0.34) 9%, rgba(29,255,0,0) 100%)" } },
                                    _react2.default.createElement(
                                        'span',
                                        { style: { marginLeft: "10px", minHeight: "100%" } },
                                        _react2.default.createElement(_LazyImage2.default, { unloadedSrc: "https://www.google.com/s2/favicons?domain=" + track.domain })
                                    ...
                                    _react2.default.createElement(
                                        'span',
                                        { style: (_ref2 = { color: "white", flex: 1, lineHeight: "10px" }, _defineProperty(_ref2, 'lineHeight', "10px"), _defineProperty(_ref2, 'textAlign', "left"), _defineProperty(_ref2, 'marginLeft', "10px"), _defineProperty(_ref2, 'paddingTop', "2px"), _ref2) },
                                        _react2.default.createElement(
                                            'span',
                                            { style: { fontWeight: "bold", fontSize: "12px", textDecoration: "line-through", color: "white" } },
                                            sub.text ? sub.text.substring(0, 30) + (sub.text.length > 30 ? "..." : "") : sub.name
                                        ),
                                        _react2.default.createElement('br', null),
                                        _react2.default.createElement(
                                            'span',
                                            { style: { fontSize: "9px", color: "#f7e6d2" } },
                                            track.name ? track.name.substring(0, 35) + (track.name.length > 35 ? "..." : "") : track.name
                                        )
                                    ),
                                    _react2.default.createElement(
                                        'span',
                                        { title: 'recheck', onClick: _this2.refreshTracking.bind(_this2, track.guid), style: { padding: "0px 5px", color: "white", cursor: "pointer" } },
                                        '\u21BB'
                                    ),
                                    _react2.default.createElement(
                                        'span',
                                        { title: 'remove track', onClick: _this2.removeTracking.bind(_this2, track.guid), style: { padding: "0px 5px", color: "white", cursor: "pointer" } },
                                        '\xD7'
                                    )
                                );
                            });
...
var TrackPrice = function (_React$Component) {
    _inherits(TrackPrice, _React$Component);

    function TrackPrice() {
        _classCallCheck(this, TrackPrice);

        var _this = _possibleConstructorReturn(this, (TrackPrice.__proto__ || Object.getPrototypeOf(TrackPrice)).call(this));
        ...
        _this.state = {
            ...
            flip: false,
            pattern: {
                firstSelector: '[itemprop="price"]',
                secondSelector: '[itemprop="lowPrice"]',
                remotedSelectors: '',
                filter: null,
                tags: ''
            }
    ...
    _createClass(TrackPrice, [{
        ...
        value: function componentDidMount() {
            ...
            chrome.tabs.query({ active: true, lastFocusedWindow: true }, function (tab) {
                ...
                for (var i = 0; i < pattern.length; i++) {
                    if (tab[0].url.match(new RegExp(pattern[i].mat, "i"))) {
                        _this2.setState({ pattern: {
                                firstSelector: pattern[i].firstSelector,
                                secondSelector: pattern[i].secondSelector,
                                remotedSelectors: pattern[i].remotedSelectors,
                                filter: pattern[i].filter,
                                tags: pattern[i].tags
                            } });
                    }
                }

                chrome.tabs.sendMessage(tab[0].id, { name: 'priceOfExistence', pattern: _this2.state.pattern }, function (response) {
                    if (response && response.price) {
                        _this2.setState({ showButtom: true, price: response.price, image: response.image, flip: response.flip });
                    }
                });
            });
        }
        ...
        value: function createTrack() {
            ...
            var ogImage = this.state.image;

            if (this.state.flip) {
                var firstSelector = pattern.firstSelector;

                pattern.firstSelector = pattern.secondSelector;
                pattern.secondSelector = firstSelector;
            }
            ...
            chrome.tabs.query({ active: true, currentWindow: true }, function (response) {
                ...
                chrome.runtime.sendMessage({
                    ...
                    subTracks: [{
                        firstSelector: pattern.firstSelector,
                        secondSelector: pattern.secondSelector,
                        remotedSelectors: pattern.remotedSelectors,
                        text: price,
                        filter: pattern.filter,
                        tags: pattern.tags
                    }],
                    pintab: pattern.pintab,
                    url: t.url,
                    scroll: 0,
                    title: t.title,
                    firstCheck: false,
            ...
            chrome.tabs.query({ active: true, lastFocusedWindow: true }, function (tab) {
                chrome.tabs.sendMessage(tab[0].id, { name: 'saveTrack' }, function (response) {});
            });
        }
    }, {
        key: 'render',
...
exports.getPopupTracks = getPopupTracks;
function getPopupTracks() {
    var successful = [];
    var fail = [];

    var guids = localStorage['guids'] ? localStorage['guids'] : '{}';

    guids = JSON.parse(guids);
    ...
    for (var j in guids) {
        ...
        try {
            tr = JSON.parse(localStorage[j]);
        } catch (e) {
            tr = {};
        }

        if (successful.length < 5) {
            for (var i in tr.subTracks) {
                if (tr.subTracks[i].changes) {
                    tr.subTracks = [tr.subTracks[i]];
                    successful.push(tr);
                    break;
                }
            }
        ...
        if (fail.length < 5) {
            for (var _i in tr.subTracks) {
                if (tr.subTracks[_i].problem) {
                    tr.subTracks = [tr.subTracks[_i]];
                    fail.push(tr);
                    break;
                }
            }
    ...
    return { successful: successful, fail: fail };
}
